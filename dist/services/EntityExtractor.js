...
class EntityExtractor {
     ...
     */
    extractEntities(text, entityTypes = ['university', 'department', 'scoreType', 'language', 'numbers']) {
        const entities = [];
     ...
     */
    extractEntityType(text, entityType) {
        switch (entityType) {
            case 'university':
     ...
     */
    extractUniversityEntities(text) {
        const entities = [];
     ...
     */
    extractDepartmentEntities(text) {
        const entities = [];
        const expandedText = this.expandAbbreviations(text);
     ...
     */
    extractLanguageEntities(text) {
        const entities = [];
        const languagePatterns = [
            { pattern: /(%?\d+)\s*(?:ingilizce|ing|english)/gi, getValue: (match) => `%${match[1]} İngilizce` },
            { pattern: /(%?\d+)\s*(?:türkçe|tr|turkish)/gi, getValue: (match) => `%${match[1]} Türkçe` },
