...
class FollowUpHandler {
     ...
     */
    generateFollowUpSuggestions(sessionId, currentIntent, currentEntities) {
        const suggestions = [];
        ...
        try {
            ...
            logger_1.default.error('Error generating follow-up suggestions', { error, sessionId, currentIntent });
     ...
     */
    generateClarificationFollowUps(sessionId, intent) {
     ...
     */
    shouldOfferHelp(sessionId, userMessage) {
        ...
        if (context?.conversationHistory) {
            const clarificationCount = context.conversationHistory.filter(msg => this.containsConfusionIndicators(msg)).length;
     ...
     */
    generateNetCalculationFollowUps(entities) {
        const suggestions = [];
     ...
     */
    generateBaseScoreFollowUps(entities) {
        const suggestions = [];
        if (entities.university && entities.department) {
            suggestions.push({
                type: 'question',
     ...
     */
    generateQuotaFollowUps(entities) {
        const suggestions = [];
        if (entities.university && entities.department) {
            suggestions.push({
                type: 'question',
     ...
     */
    generateDepartmentSearchFollowUps(entities) {
        const suggestions = [];
        if (entities.university) {
            suggestions.push({
                type: 'action',
     ...
     */
    generateContextualSuggestions(entities) {
        const suggestions = [];
     ...
     */
    createClarificationSuggestion(entity, intent) {
        const questions = {
     ...
     */
    hasEngineeringInterest(entities) {
     ...
     */
    hasSocialScienceInterest(entities) {
     ...
     */
    containsConfusionIndicators(message) {
...
//# sourceMappingURL=FollowUpHandler.js.map