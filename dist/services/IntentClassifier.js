...
class IntentClassifier {
    ...
    classifyIntent(text, entities = {}) {
        ...
        logger_1.default.debug('Classifying intent', { text: normalizedText, entities });
        ...
        for (const [intent, patterns] of Object.entries(this.intentPatterns)) {
            ...
            for (const pattern of patterns) {
                ...
                if (keywordScore > 0) {
                    ...
                    matchedKeywords.push(...pattern.keywords.filter(keyword => normalizedText.includes(keyword.toLowerCase())));
                }
            }
        ...
        logger_1.default.debug('Intent classified', {
            intent,
            confidence,
            matchedKeywords,
            allScores: Object.fromEntries(Object.entries(intentScores).map(([k, v]) => [k, v.score]))
        });
        return {
            intent,
            confidence,
     ...
     */
    calculateContextScore(entities) {
        let contextScore = 0;
     ...
     */
    handleUnknownIntent(text, entities) {
    ...
    isQuestion(text) {
        ...
        return this.questionWords.some(qw => words.includes(qw) || words.some(word => word.startsWith(qw)));
    }
    /**
     ...
     */
    getIntentSuggestions(entities) {
        const suggestions = [];
        if (entities.university && entities.department) {
            suggestions.push('Bu bölüm için kaç net gerekli?', 'Taban puanı nedir?', 'Kontenjanı kaç kişi?');
        }
        else if (entities.university) {
            suggestions.push('Hangi bölümler var?', 'En popüler bölümler neler?');
        }
        else {
            suggestions.push('Hangi üniversiteyi merak ediyorsunuz?', 'Net hesaplama nasıl yapılır?', 'Taban puan sorgulama nasıl yapılır?');
        }
        return suggestions;
    }
    /**
     ...
     */
    validateClassification(classification, entities) {
