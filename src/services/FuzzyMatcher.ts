...
export class FuzzyMatcher {
  ...
  private static readonly EXACT_MATCH_BONUS = 0.3;
  private static readonly ALIAS_MATCH_BONUS = 0.2;

  /**
   * Find best matches for a query string against a list of items
   */
  static findMatches<T>(
    query: string,
    items: T[],
    getSearchFields: (item: T) => { field: string; value: string; isAlias?: boolean }[],
    limit: number = 10
  ): MatchResult<T>[] {
   ...
   */
  private static jaccardSimilarity(str1: string, str2: string, n: number = 2): number {
   ...
   */
  private static lcsSimilarity(str1: string, str2: string): number {
   ...
   */
  private static getNgrams(str: string, n: number): Set<string> {
   ...
   */
  private static hasWordBoundaryMatch(query: string, text: string): boolean {
    const queryWords = query.split(/\s+/);
    const textWords = text.split(/\s+/);

    return queryWords.some(queryWord =>
      textWords.some(textWord =>
        textWord.startsWith(queryWord) || queryWord.startsWith(textWord)
      )
    );
  }
