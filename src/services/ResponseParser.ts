...
export class ResponseParser {
  ...
  static parseResponse(content: string, intent: string): ParsedResponse {
    ...
    try {
      ...
      logger.error('Error parsing OpenAI response', { error, content });
    }
   ...
   */
  private static extractClarificationQuestions(content: string): string[] {
   ...
   */
  private static calculateContentConfidence(content: string, intent: string): number {
   ...
   */
  private static isValidQuestion(text: string): boolean {
  ...
  static validateResponse(parsed: ParsedResponse): boolean {
    ...
    const hasTurkishWords = ['için', 'olan', 'ile', 'bu', 've'].some(word => 
      parsed.content.toLowerCase().includes(word)
    );
